...
def on_press(key):
    global stop
    try:
        # Handles the macOS oddities with backslash input
...
letter_coords = [36,89,150,200,250,320,370]
letter_y = 779
...
words = sort_by_length(find_subanagrams(i))
count = 0
print(words)
print(len(words))
...
# for word in words:
#     print(stop)
#     temp_coords = {k: v.copy() for k, v in letter_coord_pairs.items()}
    
#     for letter in word:

#         x = temp_coords[letter].pop(0)
#         pyautogui.click(x, letter_y)
#         #time.sleep(delay)
